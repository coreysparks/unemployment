---
title: "CPS analysis of covid unemployment"
...
des2<-survey::svydesign(ids=~1,strata=~stname+MONTH,  weights = ~WTFINL, data=cpsdat2)
#rm(cpsdat2)
gc()
```
# Load data file
...
cpsdat2%>%
  #filter(recentloss==1)%>%
  #filter(emp=="unemp")%>%
  filter(race_eth2%in%levs[c(2,6)])%>%
...
outna%>%
 filter(race_eth2%in%levs[c(2,6)])%>%
  ...
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Asian", "Pacific Islander"))+
...
outna2%>%
  ...
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Asian", "Pacific Islander"))+facet_wrap(~sex)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
outna2%>%
  ...
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Asian", "Pacific Islander"))+facet_wrap(~educ+sex, ncol = 4)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
library(forcats)
outna2%>%
  filter(race_eth2%in%levs[c(2,6)], nsamp>30)%>%
  mutate(educ=fct_relevel(educ,"< High School","High School","Some college","Bachelors +" ))%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2), lwd=2)+
  #scale_fill_discrete(name=)+
  labs(title = "Percent Unemployed by Race/Ethnicity, Gender and Education",subtitle = "February to May 2020",
...
outna2%>%
  ...
  labs(title = "Percent Unemployed by Race/Ethnicity, and Age Group",subtitle = "February to October 2020",
       ...
       x = "Month",
       y = "Percent Unemployed")+ylim(c(0, .4))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Asian", "Pacific Islander"))+facet_wrap(~Age_Group, ncol = 3)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
