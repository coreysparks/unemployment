...
cpsdat_lf$emp_bin<-ifelse(cpsdat_lf$emp!="curr work", 1, 0)
cpsdat_lf$month<- as.Date(as.yearmon(paste(cpsdat_lf$YEAR,cpsdat_lf$MONTH, sep="/"), format = "%Y/%m"))
cpsdat_lf$race_eth<-interaction(cpsdat_lf$race, cpsdat_lf$hisp)
library(stringr)
cpsdat_lf$race_eth2<-ifelse(str_sub(as.character(cpsdat_lf$race_eth), start = -10)=="Not Latino", as.character(cpsdat_lf$race_eth),"Latino")
#cpsdat_lf$fb<-ifelse(cpsdat_lf$CITIZEN%in%c(4,5),"Non-citizen", "Citizen")
#cpsdat_lf
cpsdat_lf$byr<-2020-cpsdat_lf$AGE
cpsdat_lf$cohort<-car::Recode(cpsdat_lf$byr, recodes = "1944:1964='boomer'; 1965:1979='genx'; 1980:1994='geny'; 1995:2015='genz'; else=NA")
cpsdat_lf$educ<-car::Recode(cpsdat_lf$EDUC, recodes = "21:72='< High School'; 73='High School'; 81:110='Some college';  111:125='Bachelors +';else=NA; 0=NA", as.factor=T)
cpsdat_lf$duremp_c<-ifelse(cpsdat_lf$DURUNEMP==999, NA, cpsdat_lf$DURUNEMP)
cpsdat_lf$fb<-ifelse(cpsdat_lf$YRIMMIG>0, "Foreign Born", "Native Born")
cpsdat_lf$avghrs<-ifelse(cpsdat_lf$AHRSWORKT==999, NA, cpsdat_lf$AHRSWORKT)
cpsdat_lf<-cpsdat_lf%>%
  select(emp_bin,CPSIDP, MISH, MONTH, WTFINL, month, race_eth2, curremp,educ, sex, race, emp, YEAR, AGE, stname, PANLWT, EDUC, avghrs, ABSENT, WKSTAT, EMPSAME, MULTJOB, NUMJOB,lfpart,recentloss,fb, cohort , METRO, duremp_c, lfpart)%>%
...
saveRDS(cpsdat_lf, file="data/cpsdat_LF.rds")

des<-survey::svydesign(ids=~1,  weights = ~WTFINL, data=cpsdat_lf)
#rm(cpsdat2)
gc()
```
# Load data file
...
```{r}
cpsdat_lf<-readRDS("data/cpsdat_LF.rds")
```
...
outna<-cpsdat_lf%>%
  ...
  summarise(#unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
         # avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          lfrate =(wtd.mean(lfpart,weights = WTFINL, na.rm=T)))%>%
 #count(lfpart, wt=WTFINL)%>%
 # filter(lfpart==1)%>%
   arrange( month)%>%
  ungroup()%>%
...
outna%>%
  filter(race_eth2!="multiple.Not Latino",race_eth2!="AIAE.Not Latino", race_eth2!="NHPI.Not Latino", race_eth2!="Asian.Not Latino")%>%
  ggplot()+geom_line(aes(x=month, y=lfrate,color=race_eth2, group=race_eth2), lwd=2)+
  #scale_fill_discrete(name=)+
  labs(title = "Labor Force Participation Rate by Race/Ethnicity",subtitle = "February to September 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Percent of Persons Age 16+")+#ylim(c(0, .2))+
...
#p
#<<<<<<< HEAD
ggsave(filename = "~/national_unemp.eps",height=8, width=10, dpi = "print" )
#=======
...
outna2<-cpsdat_lf%>%
  ...
  summarise(#unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(wtd.mean(lfpart,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
...
outna2%>%
  ...
  #scale_fill_discrete(name=)+
  labs(title = "Percent In the Labor Force by Race/Ethnicity and Gender",subtitle = "February to September 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Percent in Labor Force")+ylim(c(0, .25))+
...
#p
#<<<<<<< HEAD
ggsave(filename = "~/national_unemp_sex.eps",height=8, width=10, dpi = "print" )
#=======
...
outna2%>%
  ...
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2), lwd=2)+
  #scale_fill_discrete(name=)+
  labs(title = "Percent In the Labor Force by Race/Ethnicity, Gender and Education",subtitle = "February to May 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Percent in Labor Force")+#ylim(c(0, .6))+
...
#p
#<<<<<<< HEAD
ggsave(filename = "~/national_unemp_sex_educ.eps",height=8, width=10, dpi = "print" )
#=======
...
outna2%>%
  ...
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2), lwd=2)+
  #scale_fill_discrete(name=)+
  labs(title = "Percent In the Labor Force by Race/Ethnicity and Nativity Status",subtitle = "February to September 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Percent In the Labor Force")+ylim(c(0, .25))+
...
#p
#<<<<<<< HEAD
#ggsave(filename = "~/national_unemp_sex_fb.eps",height=8, width=10, dpi = "print" )
#=======
...
outst<-cpsdat_lf%>%
  ...
  summarise(emprate =( wtd.mean(lfpart,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          #emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)), 
          nsamp=n())%>%
  filter(nsamp>20)%>%
...
outst%>%
  #filter(stname=="New Hampshire")%>%
  ggplot(aes(x=month, y=emprate, color=race_eth2))+geom_line(aes(group=race_eth2), lwd=1.75)+ggtitle("New Hampshire Unemployment Rates", subtitle = "February to June, 2020")+geom_line(data=usavg, aes(x=month, y=emprate, group="National Rate"), col="black")+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
p<-outst%>%
  ...
  #scale_fill_discrete(name=)+
  labs(title = "Percent In the Labor Force by State and Race/Ethnicity",subtitle = "February to September 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Percent In the Labor Force")+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c(" Black", "Latino"," White"))+
...
#   ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2), lwd=2)+facet_geo(~stname, grid="us_state_grid2", label="name")+
#   #scale_fill_discrete(name=)+
#   labs(title = "Percent Unemployed by State and Race/Ethnicity",subtitle = "February to September 2020",
#        caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
#        x = "Month",
#        y = "Percent Unemployed")+
#   scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c(" Black", "Latino"," White"))+
...
outna2%>%
  ...
  labs(title = "Percent Unemployed by Race/Ethnicity, and Age Group",subtitle = "February to May 2020",
       ...
       x = "Month",
       y = "Percent Unemployed")+#ylim(c(0, .4))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c(" Black", "Latino"," White"))+facet_wrap(~Age_Group, ncol = 2)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
#p
# <<<<<<< HEAD
# ggsave(filename = "~/national_unemp_age.epsfig3
...
mutate(metro = car::Recode(METRO, recodes = "1='Not Metro';2='Central City'; 3='Outside Central City'"))%>%
  ...
  #scale_fill_discrete(name=)+
  labs(title = "Percent In the Labor Force by Metropolitan Residence",subtitle = "February to September 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Percent In the Labor Force")+#ylim(c(0, .4))+
...
#   ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2), lwd=2)+
#   #scale_fill_discrete(name=)+
#   labs(title = "Percent Unemployed byand Race/Ethnicity and Metropolitan Residence",subtitle = "February to September 2020",
#        caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
#        x = "Month",
#        y = "Percent Unemployed")+#ylim(c(0, .4))+
