...
lfpartreg%>%
 # filter(is.na(differnce)==F,  yr_month!=('2020-01-01'))%>%
  ...
  scale_color_manual( name = "Education", labels= c("< College", "Bachelors +", values = c("#004f9e","#e36c2c","#4eaf49")))
...
```{r}
byreg <- data.frame(svytable(~yr_month+emp_bin+race_eth2,subset(des, statefip == 37)))
byreg$yr_month<- as.Date(byreg$yr_month)
#View(byreg)
...
lfpartreg%>%
 # filter(is.na(differnce)==F,  yr_month!=('2020-01-01'))%>%
  ...
  geom_line(aes(x=yr_month, y=pct_ch, group = race_eth2, color=factor(race_eth2)), lwd=1.15)+
  geom_hline(yintercept = 0, lwd=.7,lty=2)+
  labs(title = "Job Gains and Losses by Race/Ethnicity and Gender",subtitle = "North Carolina - February 2020 to May 2021",
       caption = "Source: IPUMS CPS Monthly Data",
       x = "Month",
       y = "% Jobs Recovered")+
  #facet_wrap(~sex)+
 ...
 scale_color_manual( name = "Race/Ethnicity",labels=c(" Black", "Latino"," White"), values = c("#004f9e","#e36c2c","#4eaf49"))
 ggsave("C:/Users/ozd504/mc_race_gender1.png",height=8, width=10, dpi = "print")
```

```{r}
byreg <- data.frame(svytable(~yr_month+emp_bin+sex+race_eth2,subset(des, statefip == 37)))
byreg$yr_month<- as.Date(byreg$yr_month)
#View(byreg)
...
lfpartreg%>%
 ...
 ggsave("C:/Users/ozd504/mc_race_gender.png",height=8, width=10, dpi = "print")
```
