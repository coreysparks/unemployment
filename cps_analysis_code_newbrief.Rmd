...
outna2%>%
  ...
  labs(title = "Percent Unemployed Among Latinos by Nativity Status",subtitle = "February 2020 to May 2021",
       ...
       x = "Month",
       y = "Percent Unemployed")+ylim(c(0, .25))+
  scale_color_carsey( name = "Nativity",labels=c("Foreign Born", "Native Born"))+facet_wrap(~sex)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
#  geom_hline(yintercept = min(outna2$emprate[outna$month=="2021-05-01"]), lty=2, cex=.5)+
 ...
 scale_color_carsey( name = "Education",labels=c("< High School","High School","Some college","Bachelors +"))+
...
outna2<-cpsdat%>%
  ...
  arrange( month)%>%
  ungroup()%>%
  arrange( sex,educ,race_eth2, month )

#write.csv(outna2,file = "docs/unemp_to_rogelio.csv", row.names = F)
head(outna2)
library(forcats)
outna2%>%
  filter(race_eth2!="NHPI.Not Latino", race_eth2!="AIAE.Not Latino",race_eth2!="multiple.Not Latino",  race_eth2!="Asian.Not Latino",nsamp>30, is.na(educ)==F)%>%
  mutate(educ=fct_relevel(educ,"< High School","High School","Some college","Bachelors +" ))%>%
  ggplot()+
...
#  geom_hline(yintercept = min(outna2$emprate[outna$month=="2021-05-01"]), lty=2, cex=.5)+
 ...
 scale_color_carsey( name = "Education",labels=c(" Black", "Latino"," White"))+
  facet_wrap(~sex+educ, ncol = 4)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
outna2%>%
  ...
  labs(title = "Percent Unemployed Among Latinos by Nativity Status and Education",subtitle = "February 2020 to May 2021",
       ...
       x = "Month",
       y = "Percent Unemployed")+ylim(c(0, .25))+
  scale_color_carsey( name = "Nativity",labels=c("Foreign Born", "Native Born"))+facet_wrap(~sex+educ2)+scale_y_continuous(labels = scales::percent)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
