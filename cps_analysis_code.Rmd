...
save(cpsdat2,  file="~/git_area//unemployment/data/analysisdata.Rdata")
#rm(cpsdat)
gc()
```
...
library(forcats)
outna2%>%
  filter(race_eth2!="multiple.Not Hispanic", race_eth2!="NHPI.Not Hispanic", is.na(educ)==F)%>%
  ...
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Native American","Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~educ+sex, ncol = 4)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
...
outna2%>%
  ...
  filter(race_eth2!="multiple.Not Hispanic", race_eth2!="NHPI.Not Hispanic", group!="AIAE.Not Hispanic-Non-citizen")%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
...
#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "C:/Users/corey//national_unemp_sex_fb.png",height=8, width=10, dpi = "print" )
```
...
library(forcats)
outna2%>%
  filter(nsamp>30,race_eth2!="multiple.Not Hispanic", race_eth2!="NHPI.Not Hispanic", is.na(educ)==F)%>%
  mutate(educ=fct_relevel(educ,"< High School","High School","Some college","Bachelors +" ))%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate and Race/Ethnicity, Gender and Education",subtitle = "December 2019 to May 2020",
