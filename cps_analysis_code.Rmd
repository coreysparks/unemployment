---
title: "cps analysis covid"
author: "Corey Sparks, PhD"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## 

```{r}
library(ipumsr)
library(dplyr)
library(car)
library(zoo)
library(ggplot2)
library(questionr)
library(tidyquant)
library(fpp)
library(survey)
```
#read from original IPUMS
```{r, eval=FALSE}
<<<<<<< HEAD
ddi<-read_ipums_ddi("/home/corey/ExpanDrive/OneDrive Business//cps_00029.xml")
cpsdat2<-read_ipums_micro(ddi)
cpsdat2<-zap_labels(cpsdat2)
```

```{r, eval=FALSE}

stfips<-data.frame(stfip=unique(tidycensus::fips_codes$state_code), stname=unique(tidycensus::fips_codes$state_name))
head(stfips)
head(unique(cpsdat2$STATEFIP))
cpsdat2$stfip<-sprintf("%02d",cpsdat2$STATEFIP)

cpsdat2<-merge(cpsdat2, stfips, by.x="stfip", by.y= "stfip")

# cntpep<-cpsdat2%>%
#   group_by(CPSIDP)%>%
#   summarise(ntime=n())%>%
#   #filter(ntime>1)%>%
#   arrange(ntime)
# 
# cpsdat2<-merge(cpsdat2, cntpep, by="CPSIDP")

# cpsdat2%>%
#   filter(ntime>1)%>%
#   select(STATEFIP, YEAR,MISH,MONTH, SERIAL, CPSIDP, AGE, SEX, RACE)%>%
#   filter(YEAR>2009, CPSIDP!=0)%>%
#    arrange(CPSIDP, MONTH,MISH)%>%
#   head(., n=200)

```

## recoding from original data
```{r, eval=FALSE}
cpsdat2<-cpsdat2%>%
  filter(YEAR >2009,AGE>16,EMPSTAT%in%c(10,12,21,22))%>%
  mutate(emp = Recode(EMPSTAT, recodes = "0 = NA; 1='af'; 10='curr work'; 12 = 'recent no job'; 20:22='unemp'"),
         sex=Recode(SEX, recodes = "1='Male'; 2='Female'; else=NA", as.factor=T),
         race = Recode (RACE, recodes = "100 = 'White'; 200='Black';300='AIAE';651='Asian';652='NHPI';802:830='multiple'; else=NA"),
         hisp= ifelse(HISPAN !=0, "Hispanic", "Not Hispanic"))%>%
  mutate( curremp = ifelse(EMPSTAT%in%c(10,12) , 1, 0),
          recentloss=ifelse(emp=='recent no job', 1,0))%>%
  arrange(CPSIDP, MONTH,MISH)

cpsdat2$month<- as.Date(as.yearmon(paste(cpsdat2$YEAR,cpsdat2$MONTH, sep="/"), format = "%Y/%m"))
cpsdat2$race_eth<-interaction(cpsdat2$race, cpsdat2$hisp)
library(stringr)
cpsdat22$race_eth2<-ifelse(str_sub(as.character(cpsdat22$race_eth), start = -12)=="Not Hispanic", as.character(cpsdat22$race_eth),"Hispanic")
cpsdat22$fb<-ifelse(cpsdat22$CITIZEN%in%c(4,5),"Non-citizen", "Citizen")
cpsdat22$byr<-2020-cpsdat22$AGE
cpsdat22$cohort<-car::Recode(cpsdat22$byr, recodes = "1944:1964='boomer'; 1965:1979='genx'; 1980:1994='geny'; 1995:2015='genz'; else=NA")
cpsdat22$educ<-car::Recode(cpsdat22$EDUC, recodes = "21:72='< High School'; 73='High School'; 81:110='Some college';  111:125='Bachelors +';else=NA; 0=NA", as.factor=T)

cpsdat22<-cpsdat22%>%
  dplyr::select(CPSIDP, MISH, MONTH, WTFINL, month, race_eth2, curremp, sex, race, emp, YEAR, AGE, stname,  EDUC, WKSTAT,recentloss,fb, cohort , METRO, educ)%>%
  dplyr::filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
cpsdat2$race_eth2<-ifelse(str_sub(as.character(cpsdat2$race_eth), start = -12)=="Not Hispanic", as.character(cpsdat2$race_eth),"Hispanic")
cpsdat2$fb<-ifelse(cpsdat2$CITIZEN%in%c(4,5),"Non-citizen", "Citizen")
cpsdat2$byr<-2020-cpsdat2$AGE
cpsdat2$cohort<-car::Recode(cpsdat2$byr, recodes = "1944:1964='boomer'; 1965:1979='genx'; 1980:1994='geny'; 1995:2015='genz'; else=NA")

cpsdat2$educ<-car::Recode(cpsdat2$EDUC, recodes = "21:72='< High School'; 73='High School'; 81:110='Some college';  111:125='Bachelors +';else=NA; 0=NA", as.factor=T)
cpsdat2<-cpsdat2%>%
  select(CPSIDP, MISH, MONTH, WTFINL, month, race_eth2, curremp,educ, sex, race, emp, YEAR, AGE, stname, PANLWT, EDUC, AHRSWORKT, ABSENT, WKSTAT, EMPSAME, MULTJOB, NUMJOB,recentloss,fb, cohort , METRO)%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #filter(AGE>18)%>%
  group_by(CPSIDP)%>%
  arrange(CPSIDP, MONTH)
```

```{r}
#head(cpsdat2)
#rm(cpsdat2)
#rm(cpsdat2)
#rm(cntpep)

save(cpsdat22,  file="~/git_area//unemployment/data/analysisdata.Rdata")
#rm(cpsdat2)
gc()
```

```{r, eval=FALSE}
load("~/git_area/unemployment/data/analysisdata.Rdata")
```

### national rate map


```{r}

outna<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, race_eth2 )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
         # avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( race_eth2, month )

head(outna)

outna%>%
  filter(race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic")%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate by Race/Ethnicity",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+ylim(c(0, .2))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "~/national_unemp.png",height=8, width=10, dpi = "print" )
```

## gender
```{r}
outna2<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, race_eth2, sex )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( race_eth2,sex, month )

head(outna2)

outna2%>%
    filter(race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic")%>%

  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate and Race/Ethnicity and Gender",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+ylim(c(0, .25))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~sex)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "~/national_unemp_sex.png",height=8, width=10, dpi = "print" )
```

## gender x race X education
```{r}
outna2<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, race_eth2, educ, sex )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( race_eth2,sex,educ, month )

head(outna2)
library(forcats)
outna2%>%
    filter(race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic", is.na(educ)==F)%>%

  mutate(educ=fct_relevel(educ,"< High School","High School","Some college","Bachelors +" ))%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate and Race/Ethnicity, Gender and Education",subtitle = "December 2019 to May 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+#ylim(c(0, .6))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~educ+sex, ncol = 4)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "~/national_unemp_sex_educ.png",height=8, width=10, dpi = "print" )
```


## foreign born
```{r}
outna2<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, race_eth2, fb )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
         # avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( race_eth2,fb, month )

head(outna2)

outna2%>%
  mutate(group=paste(race_eth2, fb, sep = "-"))%>%
  filter(race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic", group!="AIAE.Not Hispanic-Non-citizen")%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate and Race/Ethnicity and Citizenship Status",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+ylim(c(0, .4))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~fb)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "~/national_unemp_sex_fb.png",height=8, width=10, dpi = "print" )
```

### State rate map
```{r}
library(geofacet)
outst<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, stname)%>%
  summarise(emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( stname, month )

p<-outst%>%
  filter(emprate!=0)%>%
  #filter(emprate!=0)%>%
  ggplot()+geom_line(aes(x=month, y=emprate, color=emprate))+facet_geo(~stname, grid="us_state_grid2", label="name")+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate by State",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+
  scale_color_viridis_c(name="Unemployment Rate",option="C",  direction=1)+
  ylim(c(0, .4))+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

p
ggsave(plot = p,filename = "~/stunemp_norace.png",height=12, width=16, dpi = "print" )
head(outst)

# outst%>%
#   group_by(month, race_eth2)%>%
#   summarise(max=max(emprate))

```

```{r}
outst<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, race_eth2, stname)%>%
  summarise(emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)), 
          nsamp=n())%>%
  filter(nsamp>30)%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( race_eth2,stname, month )

p<-outst%>%
  filter(race_eth2!="NHPI.Not Hispanic", race_eth2!="AIAE.Not Hispanic",race_eth2!="multiple.Not Hispanic", emprate!=0)%>%
  #filter(emprate!=0)%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+facet_geo(~stname, grid="us_state_grid2", label="name")+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate by State and Race/Ethnicity",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+
  ylim(c(0, .45))+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

  p$labels$colour<-"Race/Ethnicity"
p

# p<-outst%>%
#   filter(race_eth2!="NHPI.Not Hispanic", race_eth2!="AIAE.Not Hispanic",race_eth2!="multiple.Not Hispanic", emprate!=0)%>%
#   #filter(emprate!=0)%>%
#   ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+facet_geo(~stname, grid="us_state_grid2", label="name")+
#   #scale_fill_discrete(name=)+
#   labs(title = "Unemployment Rate by State and Race/Ethnicity",subtitle = "December 2019 to June 2020",
#        caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
#        x = "Month",
#        y = "Unemployment Rate")+
#   scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+
#   ylim(c(0, .45))+
#   #guides(fill=guide_legend(title="Race/Ethnicity"))+
#   #geom_hline(yintercept = 0, col="red", lwd=1.1)+
# theme_minimal()+theme(axis.text.x = element_text(angle = 45))

  p$labels$colour<-"Race/Ethnicity"
p
ggsave(plot = p,filename = "~/stunemp.png",height=12, width=16, dpi = "print" )
```
## gender x race X education
```{r}
outna2<-cpsdat2%>%
  filter(is.na(race_eth2)==F)%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month, race_eth2, educ, sex )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)),
          nsamp=n())%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( race_eth2,sex,educ, month )

write.csv(outna2,file = "~/unemp_to_rogelio.csv", row.names = F)
head(outna2)
library(forcats)
outna2%>%
  filter(nsamp>30,race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic", is.na(educ)==F)%>%
  mutate(educ=fct_relevel(educ,"< High School","High School","Some college","Bachelors +" ))%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate and Race/Ethnicity, Gender and Education",subtitle = "December 2019 to May 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+ylim(c(0, .4))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~educ+sex, ncol = 4)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "~/national_unemp_sex_educ.png",height=8, width=10, dpi = "print" )
```

## metro status
```{r}
outna2<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
   filter(METRO!=0, METRO!=4)%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month,  METRO )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( METRO, month )

outna2%>%
mutate(metro = car::Recode(METRO, recodes = "1='Not Metro';2='Central City'; 3='Outside Central City'"))%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=metro, group=metro))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate by Metropolitan Residence",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+#ylim(c(0, .4))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Location")+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))
ggsave(filename = "~/national_metro.png",height=8, width=10, dpi = "print" )

```

```{r}
#p$labels$colour<-"Race/Ethnicity"
#p

# head(outna2)
# 
# outna2%>%
#   filter(race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic")%>%
#   filter(METRO!=0, METRO!=4)%>%
#   mutate(metro = car::Recode(METRO, recodes = "1='Not Metro';2='Central City'; 3='Outside Central City'"))%>%
#   ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
#   #scale_fill_discrete(name=)+
#   labs(title = "Unemployment Rate byand Race/Ethnicity and Metropolitan Residence",subtitle = "December 2019 to June 2020",
#        caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
#        x = "Month",
#        y = "Unemployment Rate")+#ylim(c(0, .4))+
#   scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~metro)+
#   #guides(fill=guide_legend(title="Race/Ethnicity"))+
#   #geom_hline(yintercept = 0, col="red", lwd=1.1)+
# theme_minimal()+theme(axis.text.x = element_text(angle = 45))
# 
# #p$labels$colour<-"Race/Ethnicity"
# #p
# ggsave(filename = "~/national_metro_race.png",height=8, width=10, dpi = "print" )
# 
outna2<-cpsdat2%>%
  filter(is.na(race_eth2)==F, month>=as.Date('2019-12-01'))%>%
   filter(METRO!=0, METRO!=4)%>%
  #mutate(Group = ifelse(stname=="Texas", "Texas","Rest of US"))%>%
  group_by(month,  METRO, sex, race_eth2 )%>%
  summarise(unemprate =(wtd.mean(recentloss,weights = WTFINL, na.rm=T)),
          #avghrs=wtd.mean(AHRSWORKT,weights = WTFINL, na.rm=T),
          emprate =(1- wtd.mean(curremp,weights = WTFINL, na.rm=T)))%>%
  arrange( month)%>%
  ungroup()%>%
  arrange( METRO, month )

head(outna2)

outna2%>%
  filter(race_eth2!="multiple.Not Hispanic",race_eth2!="AIAE.Not Hispanic", race_eth2!="NHPI.Not Hispanic")%>%
  filter(METRO!=0, METRO!=4)%>%
  mutate(metro = car::Recode(METRO, recodes = "1='Not Metro';2='Central City'; 3='Outside Central City'"))%>%
  ggplot()+geom_line(aes(x=month, y=emprate,color=race_eth2, group=race_eth2))+
  #scale_fill_discrete(name=)+
  labs(title = "Unemployment Rate byand Race/Ethnicity and Metropolitan Residence",subtitle = "December 2019 to June 2020",
       caption = "Source: IPUMS CPS Monthly Data \n Calculations by Corey S. Sparks, Ph.D.",
       x = "Month",
       y = "Unemployment Rate")+#ylim(c(0, .4))+
  scale_color_brewer(type="qual", palette = "Set1", name = "Race/Ethnicity",labels=c("Non Hispanic Asian", "Non Hispanic Black", "Hispanic","Non Hispanic White"))+facet_wrap(~sex+metro)+
  #guides(fill=guide_legend(title="Race/Ethnicity"))+
  #geom_hline(yintercept = 0, col="red", lwd=1.1)+
theme_minimal()+theme(axis.text.x = element_text(angle = 45))

#p$labels$colour<-"Race/Ethnicity"
#p
ggsave(filename = "~/national_metro_sex.png",height=8, width=10, dpi = "print" )

```
